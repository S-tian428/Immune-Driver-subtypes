...
#library(affy)
#读取验证集数据GSE15460
setwd("G:/免疫亚型分型/result6/GSE15460")
GSE15460 = getGEO('GSE15460', destdir=".", AnnotGPL = F, getGPL = F) 
save(GSE15460,file = 'GSE15460.gset.Rdata')
#load("GSE15460.gset.Rdata")
######exp1(GPL570)######
###查看平台信息
GSE15460[["GSE15460_series_matrix.txt.gz"]]@annotation
...
####和marker基因匹配上的基因名保留
clinical<-pData(GSE15460[[1]])
#筛选出有生存的样本
Clinical_outcome<-read.table("GSE15460_Outcome.txt", 
                             header = TRUE, sep = "\t", stringsAsFactors = FALSE)
Clinical_outcome<-Clinical_outcome[Clinical_outcome$GSM.ID%in%colnames(dat),]
GSE15460_exp<-dat[,Clinical_outcome$GSM.ID]
save(GSE15460_exp,file = "GSE15460_NormExp.rda")

#load("H:/免疫亚型分型/MOVICS/STAD.multiomics.rda")
#STAD.surv<-STAD_multiomics$surv.info
GSE15460_clin.info<-data.frame(fustat=Clinical_outcome$Outcome..1.dead.,
                               futime=Clinical_outcome$Overall.Survival..Months...
                              )
rownames(GSE15460_clin.info)<-Clinical_outcome$GSM.ID
GSE15460_clin.info$futime<-round(GSE15460_clin.info$futime*30,0)
save(GSE15460_clin.info,file = "GSE15460_clin.info.rda")
...
library(MOVICS)
getwd()
yau.ntp.pred <- runNTP(expr       = GSE15460_exp,
...
setwd("G:/免疫亚型分型/result6/GSE15460/Oncopredict")
...
calcPhenotype(trainingExprData=trainingExprData,
              ...
              testExprData=as.matrix(GSE15460_exp),
              batchCorrect="eb",
